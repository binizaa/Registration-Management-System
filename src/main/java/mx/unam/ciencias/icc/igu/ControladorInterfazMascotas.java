 ...
 */
public class ControladorInterfazMascotas {
    ...
    /* La tabla. */
    @FXML private TableView<Mascota> tabla;

    /* La ventana. */
    private Stage escenario;
    /* El modelo de la selección. */
    private TableView.TableViewSelectionModel<Mascota> modeloSeleccion;
    /* La selección. */
    private ObservableList<TablePosition> seleccion;
    /* Los renglones en la tabla. */
    private ObservableList<Mascota> renglones;

    /* La base de datos. */
    private BaseDeDatosMascotas bdd;
    /* La conexión del cliente. */
    private Conexion<Mascota> conexion;
    ...
    @FXML private void agregaMascota(ActionEvent evento) {
        DialogoEditaMascota dialogo;
        try {
            dialogo = new DialogoEditaMascota(escenario, null);
        } catch (IOException ioe) {
            String mensaje = ("Ocurrió un error al tratar de " +
                              "cargar el diálogo de mascota.");
        ...
        if (!dialogo.isAceptado())
            return;
        bdd.agregaRegistro(dialogo.getMascota());
        try {
            conexion.enviaMensaje(Mensaje.REGISTRO_AGREGADO);
            conexion.enviaRegistro(dialogo.getMascota());
        } catch (IOException ioe) {
            dialogoError("Error con el servidor",
                         "No se pudo enviar una mascota a agregar.");
        }
    }

    /* Edita un mascota. */
    @FXML private void editaMascota(ActionEvent evento) {
        Mascota mascota = renglones.get(seleccion.get(0).getRow());
        DialogoEditaMascota dialogo;
        try {
            dialogo = new DialogoEditaMascota(escenario, mascota);
        } catch (IOException ioe) {
            String mensaje = ("Ocurrió un error al tratar de " +
                              "cargar el diálogo de mascota.");
            dialogoError("Error al cargar interfaz", mensaje);
        ...
        try {
            ...
            conexion.enviaRegistro(mascota);
            conexion.enviaRegistro(dialogo.getMascota());
        } catch (IOException ioe) {
            dialogoError("Error con el servidor",
                         "No se pudieron enviar mascotas a modificar.");
        }
        bdd.modificaRegistro(mascota, dialogo.getMascota());
    }

    /* Elimina uno o varios mascotas. */
    @FXML private void eliminaMascotas(ActionEvent evento) {
        int s = seleccion.size();
        String titulo = (s > 1) ?
            "Eliminar mascotas" : "Eliminar mascota";
        String mensaje = (s > 1) ?
            "Esto eliminará a las mascotas seleccionados" :
            "Esto eliminará a la mascota seleccionado";
        String aceptar = titulo;
        String cancelar = (s > 1) ?
            "Conservar mascotas" : "Conservar mascota";
        if (!dialogoDeConfirmacion(titulo, mensaje, "¿Está seguro?",
                                   aceptar, cancelar))
            return;
        Lista<Mascota> aEliminar = new Lista<Mascota>();
        for (TablePosition tp : seleccion)
            aEliminar.agregaFinal(renglones.get(tp.getRow()));
        modeloSeleccion.clearSelection();
        for (Mascota mascota : aEliminar) {
            bdd.eliminaRegistro(mascota);
            try {
                conexion.enviaMensaje(Mensaje.REGISTRO_ELIMINADO);
                conexion.enviaRegistro(mascota);
            } catch (IOException ioe) {
                dialogoError("Error con el servidor",
                             "No se pudo enviar una mascota a eliminar.");
                return;
            }
        }
    }
    ...
    /* Busca mascotas. */
    @FXML private void buscaMascotas(ActionEvent evento) {
        DialogoBuscaMascotas dialogo;
        try {
            dialogo = new DialogoBuscaMascotas(escenario);
        } catch (IOException ioe) {
            ioe.printStackTrace();
            String mensaje = ("Ocurrió un error al tratar de " +
    ...
    @FXML private void acercaDe(ActionEvent evento) {
        ...
        dialogo.setTitle("Acerca de Administrador de Mascotas");
        dialogo.setHeaderText(null);
        dialogo.setContentText("Aplicación para administrar mascotas.\n"  +
    ...
    private void conectar() {
        ...
        try {
            ...
            conexion = new Conexion<Mascota>(bdd, enchufe);
    ...
    private void eventoBaseDeDatos(EventoBaseDeDatos evento,
        ...
        case REGISTRO_AGREGADO:
            Platform.runLater(() -> agregaMascota(mascota1));
            break;
        case REGISTRO_ELIMINADO:
            Platform.runLater(() -> eliminaMascota(mascota1));
            break;
    ...
    private void mensajeRecibido(Conexion<Mascota> conexion, Mensaje mensaje) {
        switch (mensaje) {
        case BASE_DE_DATOS:
    ...
    private void baseDeDatos(Conexion<Mascota> conexion) {
        try {
    ...
    private void registroAlterado(Conexion<Mascota> conexion,
                                  Mensaje mensaje) {
        Mascota e;
        try {
    ...
    private void registroModificado(Conexion<Mascota> conexion) {
        Mascota e1, e2;
        try {
    ...
    /* Agrega un mascota a la tabla. */
    private void agregaMascota(Mascota mascota) {
        renglones.add(mascota);
        tabla.sort();
    }

    /* Elimina un mascota de la tabla. */
    private void eliminaMascota(Mascota mascota) {
        renglones.remove(mascota);
        tabla.sort();
    }
}
