...
public class Arreglos {
    ...
    selectionSort(T[] arreglo, Comparator<T> comparador) {
        for(int i = 0; i < arreglo.length; i++){
            ...
            for(int j = i+1; j < arreglo.length; j++)
                if(comparador.compare(arreglo[j],arreglo[m]) < 0) //A[j] < A[m]
                    m = j;
    ...
    busquedaBinaria(T[] arreglo, T elemento, Comparator<T> comparador) {
        return auxBusqB(arreglo, elemento, comparador, 0, arreglo.length-1);
    }

    private static <T> int auxBusqB (T[] arr, T elem, Comparator<T> comparador, int ini, int fin){
        if(ini>fin) return -1;
        
        //int medio = (fin+ini) / 2; 
        int medio = (ini & fin) + ((ini ^ fin) >> 1); //Para evitar desbordamineto
        int comp = comparador.compare(arr[medio], elem);

        if(comp == 0) return medio;
        else if(comp>0) return auxBusqB(arr, elem, comparador, ini, medio - 1);
        else return auxBusqB(arr, elem, comparador, medio + 1, fin);
    }
}
