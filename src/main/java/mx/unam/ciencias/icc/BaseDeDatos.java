...
public abstract class BaseDeDatos<R extends Registro<R, C>, C extends Enum> {
    ...
    public void modificaRegistro(R registro1, R registro2) {
        if(registro1 == null || registro2 == null) throw new IllegalArgumentException("El registro es nulo");
        for (R reg : registros) {
            if(registro1.equals(reg)){
                for(EscuchaBaseDeDatos<R> escucha : escuchas)
    ...
    public void guarda(BufferedWriter out) throws IOException {
        if(out == null) return;
        for(R reg : registros) out.write(reg.seria());
    }
    ...
    public Lista<R> buscaRegistros(C campo, Object valor) {
        if (!(campo instanceof CampoMascota)) throw new IllegalArgumentException("El campo dado no es válido");
        Lista<R> lista = new Lista<>();

        for(R reg : registros){
            if(reg.casa(campo,valor)) lista.agregaFinal(reg);
        }
        return lista;
    }
